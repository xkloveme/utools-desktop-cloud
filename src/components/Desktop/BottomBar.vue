<template>
  <div
    class="tw-absolute tw-bottom-0 tw-p-3 tw-w-full tw-select-none"
    style="min-width: 600px; z-index: 1001"
  >
    <div
      class="tw-h-12 tw-w-fulltw-rounded-xl"
      style="
      ...
      >
          ...
          <div class="tw-w-full tw-h-full tw-hidden sm:tw-block">
            <div class="tw-w-full tw-h-full tw-flex tw-items-center tw-px-2">
              ...
              <Icon
                ...
                @click.native="showdesktop_clicked"
              />
              <Icon
                :srcc="'tasks'"
                mode="large"
        ...
        <div class="tw-flex-none tw-w-100 tw-h-full tw-block">
          ...
          >
            <Icon
              ...
              mode="large"
              class="tw-ml-3"
              @button_clicked="explorer_clicked"
              :tagcont="'File Manager'"
              :cfocus="$store.state.current_focus_type === 'explorer'"
              :fshow="has_explorer"
              v-if="has_explorer"
              @contextmenu.prevent.native="
                mr_clicked($event, 'explorer', has_explorer)
              "
            />
            <Icon
              :srcc="'doc'"
              mode="large"
              class="tw-ml-3"
              @button_clicked="text_clicked"
              :tagcont="'Text Editor'"
              :cfocus="$store.state.current_focus_type === 'text'"
              :fshow="has_text"
              v-if="has_text"
            ...
            <Icon
              ...
              mode="large"
              class="tw-ml-3"
              @button_clicked="browser_clicked"
              :tagcont="'Browser'"
              :cfocus="$store.state.current_focus_type === 'browser'"
              :fshow="has_browser"
              @contextmenu.prevent.native="
            ...
            <Icon
              ...
              mode="large"
              class="tw-ml-3"
              @button_clicked="music_clicked"
              :tagcont="'Music'"
              :cfocus="$store.state.current_focus_type === 'music'"
              :fshow="has_music"
              @contextmenu.prevent.native="
            ...
            <Icon
              ...
              mode="large"
              class="tw-ml-3"
              @button_clicked="vscode_clicked"
              :tagcont="'vscode'"
              :cfocus="$store.state.current_focus_type === 'vscode'"
              :fshow="has_vscode"
              @contextmenu.prevent.native="
                mr_clicked($event, 'vscode', has_vscode)
              "
            />
            <Icon
              :srcc="'settings'"
              mode="large"
              class="tw-ml-3"
              @button_clicked="settings_clicked"
              :tagcont="'Settings'"
              :cfocus="$store.state.current_focus_type === 'settings'"
              :fshow="has_settings"
              @contextmenu.prevent.native="
                mr_clicked($event, 'settings', has_settings)
              "
            />
            <Icon
              :srcc="'shell'"
              mode="large"
              class="tw-ml-3"
              @button_clicked="terminal_clicked"
              :tagcont="'Terminal'"
              :cfocus="$store.state.current_focus_type === 'terminal'"
              :fshow="has_terminal"
              @contextmenu.prevent.native="
                mr_clicked($event, 'terminal', has_terminal)
              "
            />
            <Icon
              :srcc="'game'"
              mode="large"
              class="tw-ml-3"
              @button_clicked="game_clicked"
              :tagcont="'game'"
              :cfocus="$store.state.current_focus_type === 'game'"
              :fshow="has_game"
              @contextmenu.prevent.native="mr_clicked($event, 'game', has_game)"
            />
          </div>
        </div>
        <div
        ...
        >
          ...
          <div
            ...
            "
          >
            <div class="tw-flex-none tw-py-0 tw-my-0 tw-font-bold tw-text-xl">
              {{ PrefixZero(date_hour, 2) }}:{{ PrefixZero(date_minute, 2) }}
            </div>
            <div class="tw-absolute tw-text-xs tw-font-bold" style="top: 52%">
...
export default {
  ...
  methods: {
    ...
    game_clicked() {
      if (this.has_game) {
        if (this.window_type_all_hide_or_at_the_top({ type: 'game' })) {
          this.$store.commit('switch_global_window_show_status', {
            type: 'game',
          })
        }
      } else {
        this.$store.commit('open_new_window', { type: 'game' })
      }
      this.$store.commit('refresh_window_focus', { type: 'game' })
    },
    vscode_clicked() {
      if (this.has_vscode) {
