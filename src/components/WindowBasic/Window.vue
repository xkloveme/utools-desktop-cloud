<template>
  <div
    ref="window_mainbody"
    class="tw-absolute animate__animated animate__faster"
    :class="{
      animate__zoomOut: closed,
      animate__zoomOutDown: minimized,
      animate__zoomIn: !minimized,
    }"
    style="
      min-width: 680px;
      min-height: 600px;
  ...
  >
    <div
      ...
      :style="{
        ...
        'background-color': blacktheme ? '#111111' : '#f8f8f8',
      }"
    ...
    >
      ...
      >
        ...
        <div
          class="tw-w-full tw-h-10 tw-select-none"
          :style="{ 'background-color': blacktheme ? '#282828' : '#fcfcfc' }"
          :class="{ 'tw-rounded-t-xl': !full_windowed }"
        >
          <div
            class="tw-w-full tw-h-4 tw-absolute tw-overflow-hidden"
            style="left: -0px; pointer-events: none; opacity: 0.5"
            :class="{ 'tw-rounded-xl': !full_windowed }"
          >
            <div
              class="tw-w-full tw-h-10 tw-absolute"
              style="pointer-events: none"
              :style="{
                'box-shadow': blacktheme
            ...
            >
              <div
                class="
                  ...
                  tw-overflow-hidden
                "
                style="padding-right: 2px; pointer-events: auto"
                :class="{ 'tw-rounded-t-3xl': !full_windowed }"
                @click="refocus"
              >
                <WindowHeaderButton
                  :bname="blacktheme ? 'd2' : 'b2'"
                  :blacktheme="blacktheme"
                  @button_clicked="close_clicked"
                />
                <WindowHeaderButton
                  :bname="blacktheme ? 'd1' : 'b1'"
                  :blacktheme="blacktheme"
                  @button_clicked="fullwindow_clicked"
                />
                <WindowHeaderButton
                  :bname="blacktheme ? 'd3' : 'b3'"
                  :blacktheme="blacktheme"
          ...
          <div ref="header_content" class="tw-w-full tw-h-full"></div>
        </div>
        <div
          class="
        ...
        <div class="tw-w-full tw-flex-none tw-select-none" style="height: 0">
          ...
          >
            <WindowSider
              :mode="0"
              @mousedown.native="bottom_resize"
              v-if="!full_windowed && !fixedsize"
            />
          </div>
        </div>
      </div>
      <div class="tw-h-full tw-flex-none tw-select-none" style="width: 0">
        ...
        >
          ...
          <WindowSider
            :mode="1"
            @mousedown.native="right_resize"
            v-if="!full_windowed && !fixedsize"
          />
          <WindowSider
            :mode="2"
            @mousedown.native="bottom_right_resize"
            v-if="!full_windowed && !fixedsize"
          />
        </div>
      </div>
  ...
  mounted() {
    if (this.default_width < 0) {
      this.$refs.window_mainbody.style.width =
        Math.max(Math.min(1000, 0.6 * this.fullWidth), 680) + 'px'
      this.$emit(
        'width_changed',
        Math.max(Math.min(1000, 0.6 * this.fullWidth), 680)
      )
    } else {
